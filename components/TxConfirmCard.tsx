...
export default function TxConfirmCard({
  intent,
  onSuccess,
  onCancel,
}: TxConfirmCardProps) {
  ...
  async function handleConfirm() {
    ...
    try {
      ...
      setTxid(sig);
      setStatus("confirmed");
      onSuccess?.(sig); // ← success on send, not on confirm
      // Background confirmation — doesn't block UI
      connection.confirmTransaction(sig, "confirmed").catch(() => {
        console.warn(
          "Confirmation timeout — tx likely succeeded, check Solscan",
        );
      });
    } catch (err) {
      setStatus("error");
      ...
      <div style={card}>
        ...
        <div style={{ padding: "18px" }}>
          <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
            ...
            <span style={labelSm}>
              Getting best route for {intent.inputAmount} {intent.fromToken} →{" "}
              {intent.toToken}…
            </span>
          </div>
        </div>
    ...
    <div style={card}>
      <div
        style={{
          ...
          height: 1,
          background:
            "linear-gradient(90deg,transparent,rgba(245,158,11,.55),transparent)",
        }}
      />

      <Header
        badge={preview!.protocol}
        label="SWAP PREVIEW"
        dotColor="#22C55E"
        dotLabel="LIVE"
      />
      ...
      >
        ...
        >
          <Cell
            label="PRICE IMPACT"
            value={`${parseFloat(preview!.priceImpact).toFixed(4)}%`}
          />
          <Cell
            label="SLIPPAGE"
            value={`${(preview!.slippageBps / 100).toFixed(1)}%`}
          />
          {preview!.fee !== null && (
        ...
        {status === "confirmed" && txid && (
          <a
            href={`https://solscan.io/tx/${txid}${solscanCluster}`}
            target="_blank"
        ...
        {status !== "confirmed" && (
          <div style={{ display: "flex", gap: 8, marginTop: 2 }}>
            <button
              ...
              style={{ ...btnConfirm, opacity: isSubmitting ? 0.65 : 1 }}
              onMouseEnter={(e) => {
                if (!isSubmitting)
            ...
            <button
              onClick={onCancel}
              disabled={isSubmitting}
              style={{ ...btnCancel, opacity: isSubmitting ? 0.4 : 1 }}
            >
              CANCEL
            </button>
          </div>
  ...
  return (
    <span
      style={{
        width: 11,
        height: 11,
        border: "2px solid rgba(28,10,0,.3)",
        borderTop: "2px solid rgba(28,10,0,.9)",
        borderRadius: "50%",
        display: "inline-block",
        animation: "spin .8s linear infinite",
        marginRight: 8,
